...
description: Architecture compliance review of all branch changes (base..HEAD). Runs /architecture-review skill, creates remediation tasks or appends implementation constraints. Writes .sp-harden-findings.json so orchestrators (/sp:08-harden) can count p1/p2 findings.
...
Review scope is **all branch changes from the base branch (usually `main` or `master`) to `HEAD`**.
...
- Focus: architectural compliance (hexagonal boundaries: domain has no outward dependencies; ports are Protocols/ABCs; adapters depend inward; use cases orchestrate via ports; the plugin entry point wires adapters)
- **Style Requirements**:
...
8. Write structured findings file for orchestrators:
   ...
   with open('.sp-harden-findings.json', 'w') as f:
       json.dump({
