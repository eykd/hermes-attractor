...
The other checks (`check for added large files`, `check for merge conflicts`, `pyright-check`, `pytest-check`, and sometimes `ruff-check`) will require manual intervention.
...
- If `pyright-check` fails, delegate to @python-pyright-type-error-fixer agent to solve the problem.
