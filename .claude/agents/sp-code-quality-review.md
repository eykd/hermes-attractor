...
- Focus: general code quality. Cross-check against the project's enforced gates (`uv run ruff check .`, `uv run ruff format --check .`, `uv run pyright`, `uv run pytest --cov` at 100% coverage) — flag any change that would not survive these gates.
- **Style Requirements**:
...
- **Naming quality**: Use `/glossary` to validate:
  - Class/Protocol/type names match glossary terms
  - Domain concepts use canonical terminology
  - No synonyms are introduced in new code
  - Prominent names (especially in domain/use-case layers) correspond to glossary entries
