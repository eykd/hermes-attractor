...
- NEVER auto-fix F401 (unused imports) that have `# pyright: ignore` comments
- NEVER remove lines that contain `# pyright: ignore` comments
- Use conflict-safe workflow to prevent undoing pyright requirements

Your workflow is:
1. **FIRST**: Check pyright baseline with `uv run pyright` to establish current state
...
6. **AFTER FIXES**: Verify pyright still passes with `uv run pyright`
7. Repeat this cycle until no linting errors remain (except F401 conflicts)

Key principles:
- **CONFLICT PREVENTION**: Use `--ignore F401` to avoid removing imports pyright might need
...
- **SAFETY CHECK**: Verify pyright still passes after applying fixes
...
Your success metric is achieving a clean `uv run ruff check --ignore F401` output with ZERO linting violations (excluding F401 which may conflict with pyright).

**Final validation checklist:**
1. Run `uv run ruff check --ignore F401` → should be clean
2. Run `uv run pyright` → should still pass
3. Report any remaining F401 violations for manual review

You MUST reduce non-F401 linting errors to ZERO while preserving pyright compliance. This is NON-NEGOTIABLE. If you break pyright to fix ruff, that is INSUFFICIENT.
