...
9. **Create Beads Epic for Feature**:
   ...
   c. Parse the epic ID from the JSON response (format: `hermes-attractor-xxxx` or similar)
...
**Acceptance**: All tasks closed, tests pass (uv run pytest --cov), 100% coverage maintained" --json
# Store returned ID as IMPLEMENT_ID

br create "[sp:08-harden] Iterative review + remediation for $FEATURE_NAME" -p 2 --parent <epic-id> \
  --description "**Spec**: specs/$BRANCH/spec.md, plan.md
**Skills**: /security-review, /architecture-review, /quality-review, /glossary
...
- Integration patterns: Standard library/plugin interfaces unless specified otherwise
...
- "Components render efficiently" (framework-specific)
- "Cache hit rate above 80%" (technology-specific)
