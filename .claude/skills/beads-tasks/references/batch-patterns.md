...
br create --title "Implement user authentication" \
  --description "Add token-based auth use cases with login/logout"

br create --title "Add password hashing" \
  --description "Use Argon2id (argon2-cffi) for secure password storage"

br create --title "Create auth dependency" \
  --description "Port/adapter to verify tokens for protected use cases"

br create --title "Write auth integration tests" \
  --description "Test login, logout, and protected access via pytest"
...
epic_id=$(br create --title "User Authentication Feature" \
  --description "Complete auth system with tokens, password hashing, and verification" \
  --type epic --silent)

echo "Created epic: $epic_id"

# Create child tasks under the epic
br create --title "Implement token generation" \
  --description "Create token signing and verification functions in the domain layer" \
  --parent "$epic_id"

br create --title "Build login use case" \
  --description "LoginUseCase with email/password validation" \
  --parent "$epic_id"

br create --title "Build logout use case" \
  --description "LogoutUseCase to invalidate tokens" \
  --parent "$epic_id"

br create --title "Add auth adapter" \
  --description "Adapter to protect use cases requiring authentication" \
  --parent "$epic_id"
...
tasks=(
  "Implement user login|Add token-based authentication with email/password"
