---
name: beads-tasks
description: "Use when: (1) Creating, querying, updating, or closing beads tasks with the `br` CLI, (2) Parenting ad-hoc tasks for ralph automation, (3) Hitting `br` flag/JSON-field gotchas (--json, --claim, priorities, dep trees), (4) Filtering or syncing beads issues."
---
...
for task in \
  "Implement auth|Add token-based auth with login/logout" \
  "Add password hashing|Use Argon2id for secure password storage" \
  "Create auth dependency|Verify tokens on protected use cases" \
; do
